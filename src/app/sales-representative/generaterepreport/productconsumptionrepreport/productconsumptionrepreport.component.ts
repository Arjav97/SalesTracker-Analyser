...
@Component({
  selector: 'app-productconsumptionrepreport',
  templateUrl: './productconsumptionrepreport.component.html',
  styleUrls: ['./productconsumptionrepreport.component.css']
})
export class ProductconsumptionrepreportComponent {
  ...
  productconsumption(date1,date2):void{
    ...
    else
    {
      ...
      this.reportservice.productconsumptionforRep(new Date(date1f.getFullYear(),date1f.getMonth(),date1f.getDate(),0,0,0).toString(),new Date(date2f.getFullYear(),date2f.getMonth(),date2f.getDate(),24,0,0).toString()).subscribe(result=>{
